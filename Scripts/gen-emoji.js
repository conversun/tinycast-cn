...
//
// Usage: node Scripts/gen-emoji.js [emoji-test.txt en.json enDerived.json zh.json zhDerived.json]
...
function zhFor(glyph, annotations) {
  const entry =
    annotations[glyph] || annotations[glyph.replaceAll("\uFE0F", "")] || {};
  const name = cleanField((entry.tts || [])[0] || "");
  const keywords = [];
  for (const w of entry.default || []) {
    const term = cleanField(w);
    if (term && term !== name && !keywords.includes(term)) keywords.push(term);
  }
  return { name, keywords: keywords.slice(0, MAX_ZH_KEYWORDS) };
}

async function main() {
  const [emojiTest, annJson, derivedJson, zhJson, zhDerivedJson] = await load(
  ...
  const zhMerged = {
    ...JSON.parse(zhDerivedJson).annotationsDerived.annotations,
    ...JSON.parse(zhJson).annotations.annotations,
  };
  ...
  fs.writeFileSync(
    ...
    "// Generated by Scripts/gen-emoji.js — do not edit by hand.\n" +
      `// Unicode emoji ≤ E${MAX_EMOJI_VERSION.toFixed(1)} + curated symbols; fields: glyph|name|category|tone|keywords|zhName.\n` +
      "enum EmojiData {\n" +
      `    static let raw = """\n${body}\n"""\n` +
      "}\n",
  );
