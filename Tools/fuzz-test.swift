// Standalone test for the launcher matcher. Compiles the real Tinycast/Features/Launcher/Model/SearchRelevance.swift —
...
//       Tinycast/Platform/Pinyin.swift Tools/fuzz-test.swift -o /tmp/fuzz-test && /tmp/fuzz-test
...
struct FuzzTest {
    ...
    struct App {
        ...
        var fields: SearchFields {
            SearchFields(
                ...
                romanizedNames: Pinyin.aliases(for: name),
    ...
    static let apps: [App] = [
        ...
        // Han-script names, for the pinyin readings this fork adds.
        App(name: "\u{5fae}\u{4fe1}", bundleID: "com.tencent.xinWeChat"),
        App(name: "\u{7f51}\u{6613}\u{4e91}\u{97f3}\u{4e50}"),
        App(name: "\u{9ad8}\u{5fb7}\u{5730}\u{56fe}"),
        // Latin-named neighbours that collide with those readings.
        App(name: "Weixin"),
        App(name: "Weixin Helper"),
    ]
    ...
    // MARK: - Romanized readings (fork addition)

    static func romanizedReadings() {
        let wechat = "\u{5fae}\u{4fe1}"
        let netease = "\u{7f51}\u{6613}\u{4e91}\u{97f3}\u{4e50}"
        let amap = "\u{9ad8}\u{5fb7}\u{5730}\u{56fe}"

        check("full pinyin", Pinyin.aliases(for: wechat).first == "weixin")
        check("pinyin initials", Pinyin.aliases(for: wechat).last == "wx")
        check("polyphone context", Pinyin.aliases(for: amap).first == "gaodeditu")
        check("a Latin name yields no readings", Pinyin.aliases(for: "Safari").isEmpty)

        check("full pinyin finds the app", rank("weixin").contains(wechat), "got \(rank("weixin"))")
        check(
            "initials find the app", rank("wyyyy").first == netease, "got \(rank("wyyyy"))")

        // The reading sits under the literal match of the same kind...
        let weixin = rank("weixin")
        check("a literal name outranks a reading of it", weixin.first == "Weixin", "got \(weixin)")
        // ...but not so far under that it falls past the next kind down.
        check(
            "an exact reading outranks another name's prefix",
            above(weixin, wechat, "Weixin Helper"), "got \(weixin)")

        // The penalty is sized above LauncherRankingStore's cap for exactly this reason.
        let boosted = rank("weixin", boosts: [wechat: 4_500])
        check(
            "a learned boost cannot lift a reading over the literal name",
            boosted.first == "Weixin", "got \(boosted)")
    }
}
