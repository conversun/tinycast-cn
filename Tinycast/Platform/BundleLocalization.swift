...
enum BundleLocalization {
    ...
    nonisolated static func indexedLanguages(_ preferred: [String]) -> [String] {
        ...
        for tag in preferred + ["en"] {
            ...
            for form in [tag] + regionForms(tag) + [bare] {
    ...
    private static func regionForms(_ tag: String) -> [String] {
        let subtags = tag.split(separator: "-")
        guard let script = subtags.first(where: { $0.count == 4 && $0.allSatisfy(\.isLetter) })
        else { return [] }
        let language = Locale.Language(identifier: tag)
        guard let code = language.languageCode?.identifier else { return [] }
        let scriptLanguage = Locale.Language(identifier: "\(code)-\(script)")
        let defaultRegion = Locale.Language(identifier: scriptLanguage.maximalIdentifier).region
        return [language.region, defaultRegion].compactMap { region in
            region.map { "\(code)-\($0.identifier)" }
        }
    }
